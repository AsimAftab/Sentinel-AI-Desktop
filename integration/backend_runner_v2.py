...
class BackendRunner:
    ...
    def start(self):
        ...
        self.thread = threading.Thread(
            target=self._run_backend, daemon=True, name="SentinelBackend"
        )
        self.thread.start()
    ...
    def _run_backend(self):
        ...
        try:
            ...
            self.event_bus.emit(EventType.BACKEND_STARTING, status=BackendStatus.STARTING)
            ...
            logger.info("Event-based communication initialized")

            # Create service container and wire up shared state
            from src.utils.container import get_container  # type: ignore

            container = get_container()
            container.event_bus = self.event_bus
            container.shutdown_event = self.shutdown_event

            # Import the orchestrator
            from src.utils.orchestrator import run_sentinel_agent  # type: ignore

            # Notify ready
            self.event_bus.emit(EventType.BACKEND_READY, status=BackendStatus.READY)

            # Execute backend orchestrator (blocks until shutdown or error)
            run_sentinel_agent(container=container)

        except Exception as e:
            error_msg = f"Backend error: {str(e)}\n{traceback.format_exc()}"
            logger.error("%s", error_msg)
            self.event_bus.emit(
                EventType.BACKEND_ERROR, status=BackendStatus.ERROR, error=error_msg
            )
        finally:
            ...
            self.running = False
            self.event_bus.emit(EventType.BACKEND_STOPPED, status=BackendStatus.STOPPED)
